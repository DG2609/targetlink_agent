...
│   ├── xml_tools.py             # build_model_hierarchy, find_blocks_recursive, query_config, trace_connections, ...
...
def check_rule(model_dir: str) -> dict:
    ...
    for xml_file in sorted(Path(systems_dir).glob("system_*.xml")):
        ...
        blocks = root.findall("Block[@BlockType='TL_Inport']")
        for block in blocks:
            name = block.get("Name", "Unknown")
            sid = block.get("SID", "")
            path = f"{xml_file.stem}/{name}"

            config_node = block.find("P[@Name='OutDataTypeStr']")
            value = (config_node.text or "").strip() if config_node is not None else "NOT_FOUND"

            if value not in ("Inherit: auto", "NOT_FOUND"):
